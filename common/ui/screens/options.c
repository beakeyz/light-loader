...
int construct_optionsscreen(light_component_t **root, light_gfx_t *gfx) {
  ...
  for (uint32_t i = 0; i < l_options_len; i++) {
    ...
    case LOPTION_TYPE_BOOL:
      ...
      current_y += l_options[i].btn->parent->height +
                   (l_options[i].btn->parent->height / 2);
      break;
    case LOPTION_TYPE_STRING:
      l_options[i].input_box =
          create_inputbox(root, l_options[i].name, (char *)l_options[i].value,
                          24, current_y, 312, 26);

      current_y += l_options[i].input_box->parent->height +
                   (l_options[i].input_box->parent->height / 2);
      break;
    }
  }
